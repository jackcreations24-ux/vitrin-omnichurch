...
import { fetchLatestGitHubRelease, OFFICIAL_GITHUB_REPO } from '../services/githubReleases';
...
export const DevDashboardModal: React.FC<DevDashboardModalProps> = ({
  ...
  const [isUnlocked, setIsUnlocked] = useState(() => isDevSessionActive());
  const [rememberSession, setRememberSession] = useState(true);
  const [activeAccount, setActiveAccount] = useState(() => getActiveDevAccount());
  ...
  // GitHub Releases Live Synchronization
  const [isSyncingGitHub, setIsSyncingGitHub] = useState(false);
  const [githubSyncMsg, setGithubSyncMsg] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [githubRepoInput, setGithubRepoInput] = useState(OFFICIAL_GITHUB_REPO);
  ...
  const handleQuickLoginAsJackson = () => {
    if (rememberSession) {
      activateDevSession('Jackson Charles (jackson318638@gmail.com)');
    }
    setIsUnlocked(true);
  ...
  const handleSyncFromGitHub = async () => {
    ...
    try {
      const repo = githubRepoInput.trim() || OFFICIAL_GITHUB_REPO;
      const res = await fetchLatestGitHubRelease(repo);
      if (!res || !res.links || Object.keys(res.links).length === 0) {
        setGithubSyncMsg({
          type: 'error',
          text: `Pa gen okenn fichye .apk oswa .exe jwenn nan dènye release repozitwa "${repo}" la.`,
        });
        return;
      }
      ...
      const updatedLinks: DownloadLinks = {
        ...formLinks,
        ...(res.links.android ? { android: res.links.android } : {}),
        ...(res.links.pc ? { pc: res.links.pc } : {}),
        ...(res.links.pc32 ? { pc32: res.links.pc32 } : {}),
        ...(res.links.mac ? { mac: res.links.mac } : {}),
      };
      ...
      if (res.version && onSaveSiteTexts && formTexts) {
        const updatedTexts: SiteTextsConfig = {
          ...formTexts,
          heroBadge: `${res.version} Stable`,
        };
        setFormTexts(updatedTexts);
        onSaveSiteTexts(updatedTexts);
        saveSiteTextsToStorage(updatedTexts);
      }

      setGithubSyncMsg({
        type: 'success',
        text: `Senkronize avèk siksè vèsyon ${res.version} (${res.assetsCount} fichye: Android APK & Windows EXE)! Tout lyen yo mete ajou sou sit la.`,
      });
      setTimeout(() => setGithubSyncMsg(null), 8000);
    } catch (err: unknown) {
      setGithubSyncMsg({
        type: 'error',
        text: `Erè pandan senkronizasyon an: ${err instanceof Error ? err.message : String(err)}`,
      });
    } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {isUnlocked && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden sm:inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold bg-cyan-950/70 text-[#35c9ff] border border-cyan-400/40">
                      100% Kontwòl Nwaj
                    </span>
                  </div>
                )}
              ...
              <p className="text-xs text-blue-200/60">
                Aksè administrasyon, senkronizasyon vèsyon nan tout lang, ak jesyon sit OmniChurch.
              </p>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-8 max-w-lg mx-auto text-center space-y-5">
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-blue-950/70 via-black/60 to-cyan-950/50 border border-[#35c9ff]/30 shadow-[0_0_35px_rgba(8,124,255,0.25)] space-y-4">
              ...
              <div className="flex items-center gap-3.5 text-left">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#35c9ff] to-[#087cff] flex items-center justify-center text-white font-black text-xl shadow-[0_0_20px_rgba(8,124,255,0.6)] shrink-0">
                  JC
                </div>
                <div>
                  <h4 className="text-base font-extrabold text-white">Jackson Charles</h4>
                  <p className="text-xs text-blue-200/80 font-mono">jackson318638@gmail.com</p>
                  <p className="text-[11px] text-emerald-400 font-semibold mt-0.5 flex items-center gap-1">
                    <Check className="w-3 h-3 text-emerald-400" />
                    Devlopè Prensipal &amp; Mèt Sistèm OmniChurch
                  </p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleUnlock} className="space-y-3">
              ...
              <div className="flex items-center justify-between text-xs px-1 text-blue-200/70">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={rememberSession}
                    onChange={(e) => setRememberSession(e.target.checked)}
                    className="rounded border-white/20 bg-black/40 text-[#087cff] focus:ring-0"
                  />
          ...
          <div>
            ...
            {activeTab === 'links' && (
              <form onSubmit={handleSaveLinks} className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                ...
                {/* GitHub Releases Quick 1-Click Sync Card */}
                <div className="p-4 rounded-2xl bg-gradient-to-r from-[#0a1e3b]/90 via-[#07172e]/90 to-[#0a1e3b]/90 border border-[#35c9ff]/40 shadow-[0_4px_25px_rgba(53,201,255,0.12)] space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <div className="p-2 rounded-xl bg-[#35c9ff]/15 border border-[#35c9ff]/30 text-[#35c9ff]">
                        <RefreshCw className={`w-4 h-4 ${isSyncingGitHub ? 'animate-spin' : ''}`} />
                      </div>
                      <div>
                        <h5 className="text-sm font-bold text-white flex items-center gap-2">
                          <span>Senkronizasyon Otomatik ak GitHub Releases</span>
                          <span className="px-2 py-0.5 rounded text-[10px] font-black bg-cyan-950 border border-cyan-500/40 text-[#35c9ff]">
                            1-Klik
                          </span>
                        </h5>
                        <p className="text-[11px] text-blue-200/70">
                          Chak fwa w pibliye yon nouvo vèsyon sou GitHub, klike la a pou rekipere tout nouvo lyen .apk ak .exe yo otomatikman.
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#0055ff] to-[#00b4d8] hover:from-[#0044cc] hover:to-[#0096c7] text-white text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 disabled:opacity-50 cursor-pointer transition shrink-0"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isSyncingGitHub ? 'animate-spin' : ''}`} />
                      <span>{isSyncingGitHub ? 'N ap senkronize...' : 'Senkronize Kounye a'}</span>
                    </button>
                  </div>

                  <div className="flex items-center gap-2 pt-1 border-t border-white/5">
                    <span className="text-[11px] font-semibold text-blue-200/60 shrink-0">Repozitwa:</span>
                    <input
                      type="text"
                      value={githubRepoInput}
                      onChange={(e) => setGithubRepoInput(e.target.value)}
                      placeholder="jackcreations24-ux/vitrin-omnichurch"
                      className="bg-[#020712] border border-[#1b4882]/70 rounded-lg px-2.5 py-1 text-[11px] text-blue-100 font-mono w-full max-w-sm focus:outline-none focus:border-[#35c9ff]"
                    />
                    <a
                      href={`https://github.com/${githubRepoInput.trim() || OFFICIAL_GITHUB_REPO}/releases`}
                      target="_blank"
                      rel="noreferrer"
                      className="text-[11px] text-[#35c9ff] hover:underline flex items-center gap-1 shrink-0"
                    >
                      <ExternalLink className="w-3 h-3" />
                      <span>Wè sou GitHub</span>
                    </a>
                  </div>
            ...
            {activeTab === 'texts' && (
              <form onSubmit={handleSaveTexts} className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                {/* Dedicated Real-Time Version Synchronizer across all languages */}
                <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-[#087cff]/15 via-[#030d22] to-cyan-950/30 border border-[#35c9ff]/40 space-y-4 shadow-[0_0_30px_rgba(8,124,255,0.15)]">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#1b4882]/50 pb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
                        <h5 className="text-sm font-extrabold text-white flex items-center gap-2">
                          <span>🎛️ Nimewo Vèsyon &amp; Senkronizasyon nan Tout Lang</span>
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#087cff]/20 text-[#35c9ff] border border-[#35c9ff]/40">
                            Tan Reyèl
                          </span>
                        </h5>
                      </div>
                      <p className="text-[11px] text-blue-200/70 mt-0.5">
                        Lè w chanje nimewo vèsyon sa a, li senkronize otomatikman nan <b>Kreyòl</b>, <b>Français</b>, <b>English</b>, ak <b>Español</b> sou Navbar la, Bannè Akèy la, ak bouton telechajman yo.
                      </p>
                    </div>

                    {/* Quick Preset Badges */}
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] font-bold text-blue-200/50">Prereglaj:</span>
                      {['v1.0.0', 'v1.0.4', 'v1.1.0', 'v2.0.0', 'v2.5.0'].map((preset) => (
                        <button
                          key={preset}
                          type="button"
                          onClick={() => setFormTexts({ ...formTexts, heroBadge: `${preset} Stable` })}
                          className={`px-2 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer border ${
                            formTexts.heroBadge?.includes(preset)
                              ? 'bg-[#087cff] text-white border-cyan-400'
                              : 'bg-black/40 text-blue-200/70 border-white/10 hover:border-cyan-400/50'
                          }`}
                        >
                          {preset}
                        </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="sm:col-span-2 space-y-1.5">
                      <label className="text-xs font-bold text-white flex items-center justify-between">
                        <span>Antre Nimewo Vèsyon an (e.g. v1.0.4, v2.0.0 Stable)</span>
                        <span className="text-[10px] text-emerald-400 font-mono">Senkronizasyon Nwaj Aktif</span>
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full bg-[#020712] border border-[#35c9ff]/50 rounded-xl px-4 py-2.5 text-sm font-mono text-white focus:outline-none focus:border-cyan-300 shadow-[0_0_15px_rgba(8,124,255,0.2)]"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5 flex flex-col justify-end">
                      <button
                        type="submit"
                        className="w-full py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:brightness-110 text-white font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.3)] transition cursor-pointer"
                      >
                        <Cloud className="w-3.5 h-3.5" />
                        <span>Aplike Vèsyon Sa a Touswit</span>
                      </button>
                    </div>
                  </div>

                  {/* Multilingual Live Preview Cards */}
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    <p className="text-[11px] font-bold text-blue-200/80 flex items-center gap-1.5">
                      <Eye className="w-3.5 h-3.5 text-cyan-400" />
                      <span>Apèsi an tan reyèl jan vizitè yo wè l nan chak lang:</span>
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      <div className="p-2.5 rounded-xl bg-black/40 border border-white/10 text-center space-y-1">
                        <div className="text-[10px] font-bold text-blue-200/60 flex items-center justify-center gap-1">
                          <span>🇭🇹</span>
                          <span>Kreyòl Ayisyen</span>
                        </div>
                        <div className="px-2 py-1 rounded-lg bg-emerald-950/60 border border-emerald-500/30 text-emerald-300 font-mono text-[11px] font-bold truncate">
                          {formatLocalizedVersionBadge(formTexts.heroBadge, 'ht')}
                        </div>
                      </div>
                      ...
                      <div className="p-2.5 rounded-xl bg-black/40 border border-white/10 text-center space-y-1">
                        ...
                        <div className="px-2 py-1 rounded-lg bg-blue-950/60 border border-blue-500/30 text-blue-300 font-mono text-[11px] font-bold truncate">
                          {formatLocalizedVersionBadge(formTexts.heroBadge, 'fr')}
                        </div>
                      </div>
                      ...
                      <div className="p-2.5 rounded-xl bg-black/40 border border-white/10 text-center space-y-1">
                        ...
                        <div className="px-2 py-1 rounded-lg bg-cyan-950/60 border border-cyan-500/30 text-cyan-300 font-mono text-[11px] font-bold truncate">
                          {formatLocalizedVersionBadge(formTexts.heroBadge, 'en')}
                        </div>
                      </div>
                      ...
                      <div className="p-2.5 rounded-xl bg-black/40 border border-white/10 text-center space-y-1">
                        ...
                        <div className="px-2 py-1 rounded-lg bg-purple-950/60 border border-purple-500/30 text-purple-300 font-mono text-[11px] font-bold truncate">
                          {formatLocalizedVersionBadge(formTexts.heroBadge, 'es')}
                        </div>
                      </div>
                    </div>
                  </div>
